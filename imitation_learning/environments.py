...
class D4RLEnv():
  def __init__(self, env, absorbing: bool, load_data: bool=False, dataset=None, max_episode_steps=None):
    self.max_ep_steps = max_episode_steps
    if type(env) == str and env in ENVS:
      self.env = gym.make(f'{env}-expert-v2')
  ...
  def step(self, action: Tensor) -> Tuple[Tensor, float, bool]:
    ...
    try:
      ...
      state, reward, terminal, _, _ = self.env.step(action[0].detach().numpy()) 
    state = torch.tensor(state, dtype=torch.float32).unsqueeze(dim=0)  # Add batch dimension to state
  ...
  def max_episode_steps(self) -> int:
    if self.max_ep_steps is not None:
      return self.max_ep_steps
  ...
  def get_dataset(self, trajectories: int=0, subsample: int=1) -> ReplayMemory:
    ...
    #print("states", states, "actions", actions, "next_states", next_states, "terminals", terminals, "timeouts", timeouts)
    #print("states size", states.size(), "actions size", actions.size(), "next_states size", next_states.size(), "terminals size", terminals.size(), "timeouts size", timeouts.size())
    ...
    if self.absorbing:
      ...
      for i in range(len(states_list)):
        ...
        try:
          ...
          print(states_list[i].size(), torch.zeros(states_list[i].size(0), 1).size())
    ...
    #print(states_list, actions_list, next_states_list, terminals_list, timeouts_list, weights_list, num_trajectories)
