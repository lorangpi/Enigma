...
class Executor_Diffusion(Executor):
    ...
    def relative_obs_mapping(self, obs, action_step="PickPlace"):
        ...
        if action_step == "PickPlace":
            ...
            oracle = 1.2*np.concatenate([obs[index_obs["place_to_drop_pos"][0]:index_obs["place_to_drop_pos"][1]] - obs[index_obs["gripper_pos"][0]:index_obs["gripper_pos"][1]]+ [-10,14,0]])#[-18,10,0]]) #[-10,5,0]]) #best, 2nd
        elif action_step == "Drop":
