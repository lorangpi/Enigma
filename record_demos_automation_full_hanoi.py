...
class RecordDemos(gym.Wrapper):
    def __init__(self, env, args, randomize=True):
        ...
        try:
            ...
            obs, reward, terminated, truncated, info = self.env.step(env.action_space.sample())
        state = info['state']
        ...
        for step in self.action_steps:
            if step in self.episode_buffer.keys():
                if step not in self.data_buffer.keys():
                    self.data_buffer[step] = [(self.episode_buffer[step], self.symbolic_buffer, self.task_buffer)]
                else:
                    self.data_buffer[step].append((self.episode_buffer[step], self.symbolic_buffer, self.task_buffer))
                if 'peg' in step:
                    self.counter_peg += 1
                elif 'cube' in step:
                    self.counter_cube += 1
    ...
    def step_episode(self, obs):
        for operator in self.plan:
            self.obj_to_pick = obj_mapping[operator.split(' ')[2].lower()]
            self.place_to_drop = obj_mapping[operator.split(' ')[1].lower()]
            self.env.obj_to_pick = self.obj_to_pick
            self.env.place_to_drop = self.place_to_drop
            self.env.goal = self.obj_to_pick
            self.env.set_task((self.obj_to_pick, self.place_to_drop))
            print("Operator: ", operator, self.obj_to_pick, self.place_to_drop)
            ...
            if not(done_drop):
                return done_drop
        print("Successful episode?: ", done_drop)
        return done_drop
...
if __name__ == "__main__":
    ...
    env = GymWrapper(env)
    env = PickPlaceWrapper(env, horizon=20000, oracle=False, hanoi=True)
